...
The point of this figure is that the effect of additional scattered light could reduce \tesss photometric performance. 
For instance when the Moon is $24^\circ$ from the camera boresights the lens hood is expected to suppress $1/100^\mathrm{th}$ of a mean $3\times10^6\,\mathrm{ct/px}$ (per 2 second image) from the Moon (Fig.~\ref{fig:lens_hood_suppression}).
...
From Fig.~\ref{fig:noise_with_moon}, this corresponds to $\gtrsim\!3\times$ reduction
in photometric performance for $I_c < 12$ stars.
While this effect is highly dependent on the angle between the camera boresights and the Moon or Earth, the general picture is that for field angles $\theta \lesssim25^{\circ}$, the impact is severe.
...
To evaluate the cumulative impact of Earth and Moon crossings on \tesss Primary and Extended Missions we ask: for each camera, what fraction of the total observing time is \tess unable to operate at desired photometric precision because of Earth and Moon crossings?
...
The summarized version is that modeling Earth and Moon crossings in this manner causes a drop of $\lesssim$10\% of sub-Neptune planet detections compared to the case of not accounting for the crossings at all.
