...
\begin{enumerate}
	...
	\item $N_\mathrm{sys,extra\ planets}$: In how many systems do we find extra planets?
...
\begin{enumerate}
	...
	This figure shows what was originally noted in~\citetalias{Sullivan_2015}: \tesss original survey is incomplete. 
	There are a substantial number of planets just below the detection threshold, predominantly with $2R_\oplus < R_p <4R_\oplus$ (cf. Fig.~\ref{fig:primary_planet_yield}).
...
\end{figure}
	...
	The next-best scenarios, \npole, \nhemi\ and \shemiAvoid, all detect around 120.
	...
	Additionally, our values for the number of $0.2<S/S_\oplus<2$ planets from the primary mission are slightly revised from those of~\citetalias{Sullivan_2015}: while~\citetalias{Sullivan_2015} quote $48\pm7$  $0.2<S/S_\oplus<2$ and $R_p<2R_\oplus$ planets, we find $34 \pm 5$.
	Adopting the habitable zone of~\citet{kopparapu_habitable_2013},~\citetalias{Sullivan_2015} quoted $14\pm4$  $R_p<2R_\oplus$ planets. We find $11 \pm 3$.
	...
	Our assumptions about multiple planet system distributions are coarse -- we assume independent probability draws from single planet occurrence distributions.
	Thus our simulated planet population does not have systems of tightly packed inner planets.
	...
	With our somewhat nebulous hopes for this metric stated, the most prominent feature in its results is that \elong\ detects the fewest systems with extra planets (44, which is $39\%$ worse than the next-best). 
	...
	\todo[inline]{Is this NIRISS SOSS mode? Why would you use V band and not like... I, or something redder like K?}
	...
	Focusing our analysis to a \jwst-measured SNR is sensible given \tesss role as a `\jwst finder scope'~\citep{deming_jwst_tess_2009}.
	...
	Reference parameters for GJ 1214b are taken from~\citet{charbonneau_gj1214b_2009}: $R_p = 2.678R_\oplus$, $M_p = 6.55M_\oplus$, $R_\star = 0.211R_\odot$, $a = 0.0144\mathrm{AU}$, $V = 15.1$.
	...
	More importantly, Fig.~\ref{fig:yield_results} shows that most of the planets with `good' atmospheres have already been discovered after two years.
	The best extended missions (\shemiAvoid, \elong, \eshort\ and \hemis) boost the yield of such planets from $\sim\!100$ to $\sim\!125$.
	The worst, \npole, finds roughly 10.
	...
	This latter point simply means that if we were to assign equal weights to both relative metrics, then $N_\mathrm{new,atm}$ would impact the decision of which extended mission to perform less than $N_\mathrm{new,P<20d}$.
	
	
	\item $N_\mathrm{new,new\ stars}$:
	The observing strategy that collects the most photons from new stars should detect the most new planets about new stars.
	The \elong\ scenario dedicates 7 of a single year's 13 observing sectors to the ecliptic (where the other 5 are spent centered at the North Ecliptic Pole due to excessive Earth/Moon crossings).
	...
	We note that even though \elong\ is the scenario most prone to detecting planets about new stars, only $\sim 30\%$ of its newly detected planets are actually from `never-observed' hosts.~\footnote{Dubbing these planets and hosts `never-observed' neglects the question of overlapping fields with K2's observations (see discussion in Sec.~\ref{sec:discussion})}
