...
We find that for a third year's observing, provided \textit{TESS} satisfies basic engineering constraints, varying the direction the spacecraft points changes the relative number of newly detected, sub-Neptune radius planets by $\lesssim 30\%$.
In absolute terms, all proposed observing strategies detect roughly $1250\ R_p<4R_\oplus$ planets/year, similar to \textit{TESS}'s expected detection rate during its primary mission.
We also note that the rate of discovering planets at orbital periods $P > 20$ days roughly doubles in year 3 scenarios that heavily cover `old' sky.
In the long term, \textit{TESS} must re-observe the planets it finds during the primary mission, or else lose knowledge of when they transit as uncertainties in their ephemerides propagate in time.
